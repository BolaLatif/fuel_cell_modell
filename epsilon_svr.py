...
def TFC(X_train, y_train, epsi_range, C_range, degree_range, coef_range, gamma,  kernel):
    
    print("{}{} REGRESSION USING {} CPUs USING {} KERNEL".format(2*"\n", 10*"#", n_cpus, kernel))
    
    # create a dictionary with hyperparameters to investigate
    if kernel == "poly" :
        hp_dict = {"svr__epsilon": epsi_range,
                   "svr__C": C_range,
                   "svr__degree": degree_range,
                   "svr__coef0": coef_range,
                   "svr__gamma":gamma
                   }
    elif kernel == "rbf" :
        hp_dict = {"svr__epsilon": epsi_range,
    ...
    model = GridSearchCV(estimator  = make_pipeline(StandardScaler(), SVR(kernel=kernel, shrinking=True,
...
epsi_range = np.arange(0.1, 1, 0.1)
c_range = np.arange(0, 1, 0.1)
degree_range = np.arange(0, 10, 2)
...
# starting counter
    ...
    SOFC_Model = TFC(X_train, U_train, epsi_range, c_range, degree_range, coef_range, gamma, kernel)
...
"""
Created on Fri Nov 25 12:12:03 2022
